...
> 强制需求拆解为单文件粒度任务，逐步生成并在每个文件完成后做后置校验，避免单次生成粒度过大导致的规范污染和质量失控。
>
> **触发方式**：用户输入 `/code-gen [功能描述]` 时激活本流程。
...
- **网络请求** → 读取 `.claude/skills/carry-hub-request/SKILL.md`
- **权限控制** → 读取 `.claude/skills/carry-hub-auth/SKILL.md`
