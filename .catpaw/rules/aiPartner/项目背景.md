...
| Electron 客户端录制 | 房间主页内悬浮控件 | **`vip:pro` 专属**（后续迭代决定）。游戏过程中实时录制并分片上传，录制结束后视频自动出现在房间列表。架构：ffmpeg（`ffmpeg-static` npm 包）HLS 实时编码 → chokidar 监听切片 → 主进程 `net.fetch` 上传到后端 → 后端存 COS。鉴权：Token Push 模式，渲染进程调用 `auth:setToken` IPC 通道同步 JWT 到主进程，主进程在上传请求头注入 `Authorization: Bearer`。详见 `spec/electron-recorder.md` 和 `electron/dev-notes.md`。|
