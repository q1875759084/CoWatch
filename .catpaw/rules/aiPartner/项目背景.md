...
| Electron 客户端录制 | 房间主页内悬浮控件 | **`vip:pro` 专属**（后续迭代决定）。游戏过程中实时录制并分片上传，录制结束后视频自动出现在房间列表。架构：ffmpeg HLS 实时编码 → chokidar 监听切片 → 主进程 `net.fetch` 上传后端 → 后端存 COS。Windows 音频：`audio_capture.exe`（WASAPI Loopback）通过 pipe:0 传入 FFmpeg；macOS 静音。视频捕获：Windows 全屏用 ddagrab、窗口录制用 gfxcapture（均为 GPU 零拷贝 lavfi filter），macOS 用 avfoundation。鉴权：Token Push 模式，渲染进程调用 `auth:setToken` IPC 同步 JWT 到主进程。健壮性：窗口录制时 `window-watch.ts` 双层检测（handleFfmpegCrash 内单次枚举 + 5s 轮询兜底），目标窗口消失时优雅 stop() 而非无限重启。代码结构：`electron/handlers/recorder/`（index.ts + window-watch.ts）。详见 `spec/electron-recorder.md`。|
...
- **请求错误处理两层职责**：`request.ts` 拦截器统一格式化错误（包成含中文 message 的 `ApiError`），不做 UI 反馈；**影响页面存活的初始化请求**（失败后 UI 永远 Loading 的）必须在调用侧添加 `.catch()` 做用户可见提示（如 `message.error`）。判断标准：「失败后 UI 是否进入僵尸状态」→ 是则必须有业务侧 catch
- 视频上传前端校验（`src/utils/validateVideo.ts`）按**房间等级**区分策略：`vip:basic` — ① 读文件头 32KB 扫描 moov/mdat 顺序（moov 必须在 mdat 之前）；② 平均码率 ≤ 8 Mbps（对应 CRF 28）；`vip:pro` — 跳过以上两项，只校验文件大小 ≤ 3 GB（后端负责转码）；失败时用 antd `Modal.error()` 弹窗告知用户
