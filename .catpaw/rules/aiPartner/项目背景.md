...
| Electron 客户端录制 | 房间主页内悬浮控件 | **`vip:pro` 专属**（后续迭代决定）。游戏过程中实时录制并分片上传，录制结束后视频自动出现在房间列表。架构：ffmpeg HLS 实时编码 → chokidar 监听切片 → 主进程 `net.fetch` 上传后端 → 后端存 COS。Windows 音频：`audio_capture.exe`（WASAPI Loopback）通过 pipe:0 传入 FFmpeg。视频捕获：Windows 全屏用 ddagrab、窗口录制用 gfxcapture（均为 GPU 零拷贝 lavfi filter）。鉴权：Token Push 模式，渲染进程调用 `auth:setToken` IPC 同步 JWT 到主进程。健壮性：窗口录制时 `window-watch.ts` 双层检测（handleFfmpegCrash 内单次枚举 + 5s 轮询兜底），目标窗口消失时优雅 stop() 而非无限重启。代码结构：`electron/handlers/recorder/`（index.ts + window-watch.ts）。详见 `spec/electron-recorder.md`。|
