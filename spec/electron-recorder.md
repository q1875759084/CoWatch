...
- [x] 录制期间禁止切换房间：录制进行中时，**路由守卫拦截导航**，弹窗提示「录制进行中，请先停止录制后再切换房间」，确认后仅保留"停止录制"操作，**不允许直接离开**（含关闭应用触发的 `beforeunload` 场景，后者同时停止 ffmpeg 并触发 finish 流程）。

## 9. 已有追问
1. 录制 UI 组件放在哪里？
- 房间内悬浮，【视频列表】标题栏右侧
2. 录制控件在非 Electron 环境（普通浏览器）下如何处理？
- 渲染但置灰，hover提示"请使用客户端"
3. 录录制进行中，控件上显示什么信息？
- 🔴 00:12:34（仅时长）
4. 用户点击"开始录制"后，窗口选择在哪个时机出现？
- 点击"开始录制" → 弹窗选择目标窗口 → 确认后开录
5. 弹窗里是否显示所有窗口（包括 CoWatch 自身），还是过滤掉明显无意义的？
- 显示所有窗口，但如果列表只有 CoWatch 自身（或为空），显示提示文案"请先启动要录制的程序"
6. 点击"停止录制"后，等待所有片段上传完成期间，UI 显示什么？上传完成后自动关闭控件还是保留？
- 停止后显示上传进度，完成后控件恢复待机，房间视频列表自动出现新录屏